    ...
    <div class="riskIndex">
        <div class="buttoncrowd top_header">
            <div style="float:left;">
                ...
                <span class="risktitle active">部门列表  </span>
        ...
        <div class="buttoncrowd top_content">
            <el-table :data="tableData" :span-method="arraySpanMethod"  border stripe style="width: 100%;">
                ...
                <el-table-column align="center" prop="assetNumber" label="部门名称" >
                    <template slot-scope="scope"><span>{{scope.row.assetNumber }}</span></template>
                </el-table-column>
                <el-table-column align="center" prop="contractCode" label="部门编码" >
                    <template slot-scope="scope"><span>{{scope.row.contractCode }}</span></template>
                </el-table-column>
                <el-table-column align="center" prop="purchaseOrder" label="部门类型" >
                    <template slot-scope="scope"><span>{{scope.row.purchaseOrder }}</span></template>
                </el-table-column>
                <el-table-column align="center" prop="purchaseOrder" label="备注" >
                    <template slot-scope="scope"><span>{{scope.row.purchaseOrder }}</span></template>
                </el-table-column>
...
export default {
    ...
    methods:{
        arraySpanMethod(){},
        goAdd(num){
            this.$router.push({path:'/home/information/organizeAdd',name:'organizeAdd',query:{'ind':num}})
        },
        organize(ind){
            if(ind==1){
        ...
        detailRoute(ind,id){//编辑页面跳转
            this.$router.push({path:'/home/information/organizeAdd',name:'organizeAdd',query:{
                'ind':ind,
                'id':id
            }})
